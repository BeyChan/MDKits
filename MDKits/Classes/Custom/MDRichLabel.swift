...
public typealias MDTextClickHandler =  (String, Int) -> Void
...
public struct StringParagraph: Equatable {
    ...
    let tag: Int
    let color: UIColor?
    let clickHandler: MDTextClickHandler?
    let location: Int
...
public struct MDRect: Hashable {
    
    public static func ==(lhs: MDRect, rhs: MDRect) -> Bool {
...
public class MDRichLabel: UIView {
    ...
    fileprivate var drawText: NSAttributedString? {
        var text = texts.reduce(NSMutableAttributedString()) { (result, paragraph) -> NSMutableAttributedString in
            var attributes: [NSAttributedString.Key : Any] =  [NSAttributedString.Key.font: self.font]
            ...
            if let touchedParagraph = self.touchedParagraph {
                if isTouching && nil != paragraph.clickHandler && touchedParagraph == paragraph{
                    attributes[.backgroundColor] = highlightedColor
                }
            }
        ...
        text = NSMutableAttributedString(string: string, attributes: [NSAttributedString.Key.font: font, .foregroundColor: textColor])
        return text
    }
    
    fileprivate var rects: [MDRect : StringParagraph] = [:]
    
    public func add(text: String, tag: Int = Int.max, color: UIColor? = nil, clickHandler: MDTextClickHandler? = nil) {
        var location: Int = 0
    ...
    override public func draw(_ rect: CGRect) {
        ...
        var rects: [MDRect : StringParagraph] = [:]
        var textIndex = 0
        for (i, line) in lines.enumerated() {
            ...
            for run in runs {
                ...
                for paragraph in paragraphs {
                    ...
                    rects[MDRect(value: runRect)] = paragraph
                    MDLog("~~~~~~~")
                    MDLog("range(\(start)-\(end)) 计算宽时range(\(location),\(length))")
                    MDLog("\(drawText!.string[drawText!.string.index(drawText!.string.startIndex, offsetBy: start) ..< drawText!.string.index(drawText!.string.startIndex, offsetBy: end)])")
                    MDLog("\(runRect)")
                    MDLog("-------")
                }
