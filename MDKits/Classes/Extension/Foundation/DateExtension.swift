...
class DateClass {
    ...
    //将时间戳转换为具体时间
    static func timeStampToString(_ timeStamp:Double?,dateFormat: String = "yyyy-MM-dd HH:mm:ss")-> String? {
    ...
    // 时间戳转换，支持毫秒
    static func timeStamp(_ timeStamp : String?, dateFormat : String = "YYYY/MM/dd", isMillSecond : Bool = true) -> String? {
        if let timeInterval = TimeInterval(timeStamp ?? "") {
            let newTimeInterval = isMillSecond ? timeInterval / 1000 : timeInterval
            let df = DateFormatter()
            df.dateFormat = dateFormat
            return df.string(from: Date(timeIntervalSince1970: newTimeInterval))
        }
        return "----/--/--"
    }
    //获取系统的当前时间戳
