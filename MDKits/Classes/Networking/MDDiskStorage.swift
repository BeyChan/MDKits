...
public class MDDiskStorage {
    ...
    private static func createBaseDirectory(at path: String) throws {
        do {
            ...
            #if DEBUG
            MDLog("create cache directory failed")
            throw error
            #endif
        }
    }
        ...
        if !fileManager.fileExists(atPath: path, isDirectory: &isDir) {
            ...
            if !isDir.boolValue {
                do {
                    ...
                    #if DEBUG
                    MDLog("removeItem at Path Error")
                    #endif
                }
            }else {
                MDLog("The \(path) is Exist ")
            }
        }
    }
